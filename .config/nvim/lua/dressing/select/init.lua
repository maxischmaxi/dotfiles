...
-- schedule_wrap to avoid a bug when vim opens, and to prevent focus problems
-- for providers that close their window asynchronously
local select = vim.schedule_wrap(util.make_queued_async_fn(3, function(items, opts, on_choice)
	vim.validate("items", items, function(a)
		return type(a) == "table" and vim.islist(a)
	end, "list-like table")
	...
	elseif opts.format_item then
		-- format_item doesn't technically have to return a string for the core
		-- implementation, so wrap the return value with tostring
