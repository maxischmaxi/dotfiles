-- Telescope is configured on first use: setup plus the fzf and ui-select
-- extensions cost ~7ms at startup, so every entry point below goes through
-- ensure() first.
...
local function ensure()
	...
	-- takes over vim.ui.select
	require("telescope").load_extension("ui-select")
end

-- vim.ui.select goes through telescope-ui-select, which only installs itself
-- once the extension is loaded, so the first call has to trigger that
...
-- nvim's default grr/gri/grn/gra/grt/grx maps would make gr wait for timeoutlen
for _, keys in ipairs({ "grr", "gri", "grn", "gra", "grt", "grx" }) do
