...
# Decode a QR code from a screen region. Ported from omarchy-capture-qr.
...
if [[ -z $RESULT ]]; then
  notify-send -a screenshot -u critical "No QR code found" "Select a region containing a QR code"
  exit 1
fi
...
notify-send -a screenshot "QR code copied to clipboard"
