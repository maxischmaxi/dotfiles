...
# Pick a screen region over frozen screen content. Ported unchanged from
# omarchy-capture-region — it only needs hyprctl, jq, slurp and hyprpicker.
#
# Usage: capture-region.sh [region|windows|smart|fullscreen] [--keep-freeze] [--match-monitor]
#        capture-region.sh --take-fullscreen | --take-window
#        capture-region.sh --select-window <next|prev|left|right|up|down>
...
# surface (the layer.opened handler in hyprland.lua) invoke these modes. The
